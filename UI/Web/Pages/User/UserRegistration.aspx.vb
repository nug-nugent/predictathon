...
#End Region
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            ElseIf Me.IsPostBack AndAlso Request.Form("__EVENTTARGET") = tdLogin.UniqueID Then
                ...
                Response.Redirect("~/Pages/Common/Login.aspx", False)
            End If
        ...
        Private Sub btnSubmitUserDetails_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmitUserDetails.Click
            ...
            If OnValidate() = True Then
                ...
                If PaymentRequired() = True Then
                    ShowPaymentDetails()
                Else
                    divCompetitionDetail.Visible = False
                    CreateUserAndLogIn(String.Empty, True)
                End If
            End If
        End Sub

        Private Function PaymentRequired() As Boolean
            Return Predictathon.CompetitionManager.Load(Me.CompetitionID).EntranceFee > 0
        End Function
        ...
        Private Function OnValidate() As Boolean
            ...
            'ensure the username isn't an email address
            If blnValid Then
            ...
            If blnValid Then
                If Not IsNothing(UserManager.Load(txtUsername.Text.Trim)) Then
            ...
            'check that the email address hasn't been used by another user
            If Predictathon.Validation.IsEmailAddress(txtEmailAddress.Text) Then
            ...
            If Not String.IsNullOrEmpty(txtPassword.Text) AndAlso Not String.IsNullOrEmpty(txtConfirmPassword.Text) Then
                ...
                Else
                    ...
                    'store the (encrypted) password in a hidden field, otherwise it won't get posted back on the next trip to the server...
                    hdnp.Value = Predictathon.Security.Encryption.Encrypt(txtPassword.Text)
                End If
            End If
        ...
        Private Sub CreateUserAndLogIn(ByVal PaymentCreditCode As String, ByVal SendEmailConfirmation As Boolean)
            'create a User record
            Dim objUser As PredictathonModel.User = UserManager.Create(Guid.NewGuid, txtUsername.Text.Trim)
            With objUser
                .EmailAddress = txtEmailAddress.Text.Trim
                .Forenames = txtForenames.Text.Trim
                .Surname = txtSurname.Text.Trim
                .Password = hdnp.Value
            End With

            'create a UserCompetition record
            Dim objUserCompetition As PredictathonModel.UserCompetition = UserCompetitionManager.Create(Guid.NewGuid, objUser.UserID, Me.CompetitionID)
            With objUserCompetition
            ...
            If Not String.IsNullOrEmpty(PaymentCreditCode) Then
                ...
                Dim objPaymentCredit As PredictathonModel.PaymentCredit = PaymentCreditManager.PaymentCreditListGet(Me.CompetitionID, txtPaymentCreditCode.Text).FirstOrDefault
                If Not IsNothing(objPaymentCredit) Then
                    ...
                    objPaymentCredit.UsedByUserID = objUser.UserID
            ...
            '..now authenticate the user we've just created, and send them to the main menu
            Predictathon.Security.Authenticate(objUser.Username, String.Empty, False, True)
        ...
        Private Sub btnSubmitPaymentCreditCode_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmitPaymentCreditCode.Click
            If Validation.ValidateControl(txtPaymentCreditCode, lblPaymentCreditCode, Validation.ValidationType.NullOrEmpty) = True Then
                ...
                If PaymentCreditManager.IsValidPaymentCreditCode(Me.CompetitionID, txtPaymentCreditCode.Text) Then
                    'it's good... make sure the payment credit can't be used again, then create and log the user in
                    CreateUserAndLogIn(txtPaymentCreditCode.Text, True)
                    divCompetitionDetail.Visible = False
                Else
        ...
        Private Sub btnPayPal_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnPayPal.Click
            ...
            System.Web.HttpContext.Current.Response.Write("<input type=""hidden"" name=""first_name"" value=""" & Left(txtForenames.Text.Trim, 32) & """>") 'buyer's forenames (32 char limit)
            System.Web.HttpContext.Current.Response.Write("<input type=""hidden"" name=""last_name"" value=""" & txtSurname.Text.Trim & """>") 'buyer's surname (64 char limit)
